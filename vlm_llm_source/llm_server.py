...
# Audio settings
SAMPLE_RATE = 16000
CHUNK_MS = 100     # Smaller chunk to reduce buffer overflow
SILENCE_THRESHOLD = 0.01  # Higher threshold to filter background noise
SILENCE_CHUNKS = 10  # ~1 second of silence (10 * 100ms)
...
def record_until_silence():
    ...
    with sd.InputStream(samplerate=SAMPLE_RATE, channels=1, dtype='float32', blocksize=chunk_size, latency='high') as stream:
        while True:
            # Read chunk
...
class MPCControlScript:
    def __init__(self):
        ...
        # Initialize attributes first to avoid AttributeError on failure
        self.model = None
        self.tokenizer = None
        ...
        print(f"\n[Loading] {QWEN2_MODEL}...")
        try:
            self.model, self.tokenizer = load(QWEN2_MODEL)
            print(f"[Done] LLM loaded (MLX)")
        except Exception as e:
            print(f"[Error] Failed to load model: {e}")
            print("Make sure you have installed: pip install mlx-lm")
            return
    ...
    def generate_decision(self, command: str) -> dict:
        """Generate driving decision using local LLM (MLX)"""
